...
namespace SharpRES
{
    class Program
    {
        // All language codes recognized by the -LANG flag.
        private static readonly string[] ValidLanguages = { "EN", "FR", "IT", "DE", "ES", "RU" };
        ...
        private static void HandleUnpack(string inputFile, string[] args)
        {
            ...
            for (int i = 2; i < args.Length - 1; i++)
            {
                ...
                {
                    string candidate = args[i + 1].ToUpperInvariant();
                    if (!ValidLanguages.Contains(candidate))
                    {
                        Console.WriteLine($"Error: '{args[i + 1]}' is not a recognized language code.");
                        Console.WriteLine($"  Valid codes: {string.Join(", ", ValidLanguages)}");
                        return;
                    }
                    langFilter = candidate;
                    break;
                }
            }
            ...
            if (extension == ".res")
            {
                ...
                if (langFilter != null)
                    Console.WriteLine($"[Info] Language filter: '{langFilter}'");

                RESData printer = new RESData(
                    resFile, packageRDP, dataRDP, patchRDP, inputFile,
        ...
        /*  private static void HandleRepack(string[] args)
          ...
          private static (Dictionary<string, (FileStream, BinaryWriter, string)>, Dictionary<string, long>) PrepareRDPStreams(string targetRdpFile)
          {
              ...
              string outputRdpFile = Path.Combine(
                  Path.GetDirectoryName(targetRdpFile) ?? string.Empty,
                  Path.GetFileNameWithoutExtension(targetRdpFile) + "_new.rdp");
              try
              {
                  ...
                  cursors[targetRdpFile] = stream.Length;
        ...
        private static void PrintUsage()
        {
            ...
            Console.WriteLine($"Global Eater by Yamato Nagasaki [Experimental Release v{version}]");
            Console.WriteLine("- A GOD EATER Tool. Used for RES/RTBL Unpacking.\n");
            Console.WriteLine("Usage:");
            Console.WriteLine("  Global_EATER.exe -x [input.res|input.rtbl] [options]\n");
            Console.WriteLine("Options:");
            Console.WriteLine("  -single    Extract only the specified .res file; skip nested archives.");
            Console.WriteLine("  -LANG <code>       Extract a single language from a localized .res file.");
            Console.WriteLine($"        Valid codes: {string.Join(", ", ValidLanguages)}");
            Console.WriteLine("         If the file has 3 languages (EN, FR, IT) and a 6-language code is given (DE, ES, RU), an error is reported.");
            Console.WriteLine("         Has no effect on non-localized .res or .rtbl files.\n");
            Console.WriteLine("Examples:");
            Console.WriteLine("  Global_EATER.exe -x god_game.res");
            Console.WriteLine("  Global_EATER.exe -x god_game.res -LANG EN");
            Console.WriteLine("  Global_EATER.exe -x god_game.res -LANG FR -single");
        }
